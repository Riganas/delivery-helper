...
async function recalculateEtaForCurrentOrder() {
  ...
  try {
    ...
    el("optimizerStatus")
      .textContent =
        `✓ ETA ενημερώθηκε • ${km.toFixed(1)} km • τέλος περίπου ${hhmmFromEpoch(summary.finishEpoch)}`;
...
function mergeOneCustomer(incoming) {
  ...
  /*
    Import may refresh normal contact/address fields,
    but blank incoming fields never erase useful data.
  */
  const textFields = [
    "name",
    "taxAddress",
  ...
  /*
    ...
    - companyNotes
    Daily files cannot overwrite them.
  */
    ...
    try {
      ...
      if (lowerName.endsWith(".xlsx") || lowerName.endsWith(".xls")) {
        if (typeof XLSX === "undefined") {
          throw new Error("Δεν φορτώθηκε η βιβλιοθήκη Excel. Έλεγξε τη σύνδεση και ξαναδοκίμασε.");
        }

        const buffer = await file.arrayBuffer();
        const workbook = XLSX.read(buffer, { type: "array", cellText: true, cellDates: false });
        const sheetName = workbook.SheetNames.find(
          n => String(n).trim().toLowerCase() === "customers"
        ) || workbook.SheetNames[0];
        if (!sheetName) throw new Error("Το Excel δεν περιέχει φύλλο δεδομένων.");

        // Some Excel generators do not write a worksheet !ref/dimension.
        // In that case SheetJS sheet_to_json can return an empty grid even though
        // the cells are present. Build the grid directly from cell addresses.
        const ws = workbook.Sheets[sheetName];
        const cellRefs = Object.keys(ws || {}).filter(
          k => !k.startsWith("!") && /^[A-Z]+[0-9]+$/.test(k)
        );

        if (!cellRefs.length) {
          throw new Error(`Το φύλλο ${sheetName} άνοιξε, αλλά ο αναγνώστης Excel δεν επέστρεψε κελιά.`);
        }
        ...
        for (const ref of cellRefs) {
          ...
          let value = "";
          if (cell) {
            if (cell.w !== undefined && cell.w !== null) value = cell.w;
            else if (cell.v !== undefined && cell.v !== null) value = cell.v;
          }
          grid[pos.r][pos.c] = String(value);
        }

        const norm = v => String(v ?? "").replace(/^\uFEFF/, "").replace(/\s+/g, " ").trim().toLowerCase();
        const aliases = {
          id: ["customer id", "customerid", "id", "κωδικός πελάτη", "κωδικος πελατη"],
          name: ["name", "όνομα", "ονομα"],
          ...
          city: ["city", "πόλη", "πολη"],
          notes: ["notes", "σημειώσεις", "σημειωσεις"]
        };
        ...
        for (let r = 0; r < Math.min(grid.length, 20); r++) {
          ...
          const findCol = list => vals.findIndex(v => list.includes(v));
          const candidate = {};
          for (const [key, list] of Object.entries(aliases)) candidate[key] = findCol(list);
          if (candidate.id >= 0) { headerRow = r; col = candidate; break; }
        }
        // Fallback for XLSX files whose worksheet XML uses namespace-prefixed tags
        // that older SheetJS builds may not expose as cells. Parse the actual XLSX ZIP/XML.
        if (headerRow < 0 && typeof JSZip !== "undefined") {
          const zip = await JSZip.loadAsync(buffer);
          const shared = [];
          const ssFile = zip.file("xl/sharedStrings.xml");
          if (ssFile) {
            const ssDoc = new DOMParser().parseFromString(await ssFile.async("text"), "application/xml");
            for (const si of Array.from(ssDoc.getElementsByTagNameNS("*", "si"))) {
              shared.push(Array.from(si.getElementsByTagNameNS("*", "t")).map(n => n.textContent || "").join(""));
            }
          }
          const xmlFile = zip.file("xl/worksheets/sheet1.xml");
          if (!xmlFile) throw new Error("Δεν βρέθηκε το πρώτο φύλλο μέσα στο Excel.");
          ...
          for (const rowNode of rows) {
            const rr = Math.max(0, Number(rowNode.getAttribute("r") || 1) - 1);
            if (!xmlGrid[rr]) xmlGrid[rr] = [];
            for (const c of Array.from(rowNode.getElementsByTagNameNS("*", "c"))) {
              ...
              let value = "";
              if (type === "inlineStr") {
                value = Array.from(c.getElementsByTagNameNS("*", "t")).map(n => n.textContent || "").join("");
              } else {
          ...
          grid.length = 0;
          for (const r of xmlGrid) grid.push(r || []);
          for (let r = 0; r < Math.min(grid.length, 20); r++) {
            const vals = (grid[r] || []).map(norm);
            const findCol = list => vals.findIndex(v => list.includes(v));
            const candidate = {};
        ...
        if (headerRow < 0) {
          throw new Error(`Δεν βρέθηκε στήλη "Customer ID" στο φύλλο ${sheetName}.`);
        }
        ...
        for (let r = headerRow + 1; r < grid.length; r++) {
          ...
          const fullAddress = [addressOnly, city].filter(Boolean).join(", ");
          customers.push({
            id,
