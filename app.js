...
const FIREBASE_CONFIG = {
  apiKey: "AIzaSyDeJ8scsE4HxfQcUQ45yFoCdqeOq502DvE",
...
async function initFirebaseSync() {
  ...
  try {
    ...
    // v25.2 diagnostic: initialize Authentication first and keep Firestore
    // out of the login path until Google authentication succeeds.
    firebaseAuth = firebase.auth();

    firebaseAuth.onAuthStateChanged(user => {
      renderFirebaseUser(user);
      if (user && !firebaseDb) firebaseDb = firebase.firestore();
      cloudSetStatus(user
        ? `Firebase v${APP_VERSION}: συνδεδεμένο. Έτοιμο για δοκιμή Cloud Sync.`
        : `Firebase v${APP_VERSION}: αναμονή σύνδεσης.`);
    });

    el("googleSignInBtn").onclick = async () => {
      try {
        cloudSetStatus(`Σύνδεση με Google… • v${APP_VERSION} • ${location.origin}`);
        const provider = new firebase.auth.GoogleAuthProvider();
        provider.setCustomParameters({ prompt: "select_account" });
        await firebaseAuth.signInWithPopup(provider);
      } catch (error) {
        console.error("Delivery Helper Firebase login diagnostic", {
          appVersion: APP_VERSION,
          sdkVersion: firebase.SDK_VERSION || "unknown",
        ...
        cloudSetStatus(
          `⚠ Google login [${error?.code || "error"}] • v${APP_VERSION} • ${location.origin} • ` +
          (error?.message || error)
        );
      }
    };
