      ...
      try {
        ...
        if (lowerName.endsWith(".xlsx") || lowerName.endsWith(".xls")) {
          ...
          const preferredSheet = workbook.SheetNames.find(
            name => String(name).trim().toLowerCase() === "customers"
          ) || workbook.SheetNames[0];

          if (!preferredSheet) {
            throw new Error("Το Excel δεν περιέχει φύλλο δεδομένων.");
          }
          ...
          customers = rows.map(row => {
            const id = String(row["Customer ID"] ?? row["ID"] ?? row["CustomerID"] ?? "").trim();
            const name = String(row["Name"] ?? row["Όνομα"] ?? "").trim();
            const phone1 = String(row["Phone 1"] ?? row["Phone1"] ?? row["Τηλέφωνο 1"] ?? "").trim();
            const phone2 = String(row["Phone 2"] ?? row["Phone2"] ?? row["Τηλέφωνο 2"] ?? "").trim();
            const addressOnly = String(row["Address"] ?? row["Διεύθυνση"] ?? "").trim();
            const city = String(row["City"] ?? row["Πόλη"] ?? "").trim();
            const address = [addressOnly, city].filter(Boolean).join(", ");
        ...
        if (!Array.isArray(customers)) {
          throw new Error("Το αρχείο δεν περιέχει λίστα πελατών.");
        }
        ...
        const message =
          `✓ Συγχώνευση ολοκληρώθηκε: ${added} νέοι • ${updated} υπάρχοντες` +
          (
            skipped
