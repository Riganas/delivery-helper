...
// Reject obviously wrong first-visit geocoding results before they can
// produce multi-hour / multi-thousand-kilometre ETA calculations.
// Verified driver GPS is NEVER rejected here.
...
function rejectApproximateGpsOutliers(pending) {
  ...
  const MAX_APPROX_FROM_ROUTE_KM = 140;
  const rejected = [];
  ...
  for (const item of pending) {
    ...
    if (Number.isFinite(km) && km > MAX_APPROX_FROM_ROUTE_KM) {
      rejected.push(item);
      c.lat = null;
      c.lng = null;
  ...
  if (rejected.length) {
    ...
    console.warn("Rejected implausible approximate GPS:", missingLocationSummary(rejected));
  }
...
async function ensureApproximateGpsForStops(pending, statusText = "") {
  ...
  // A geocoder can find the right street name in the wrong part of Greece.
  // Drop implausible approximate points so a local route cannot become
  // thousands of kilometres. Verified GPS is always kept.
  rejectApproximateGpsOutliers(pending);
...
async function verifyCustomerGps(customerKey) {
  ...
  if (!confirm(
    `Είσαι στο σωστό σημείο παράδοσης για ${label};\n\nΤο GPS του κινητού θα αποθηκευτεί ως επιβεβαιωμένο σημείο.`
  )) return;

  try {
    if (el("optimizerStatus")) el("optimizerStatus").textContent = "📍 Λήψη ακριβούς GPS…";
    ...
    if (el("optimizerStatus")) {
      el("optimizerStatus").textContent =
        `✓ GPS επιβεβαιώθηκε για ${label} • ακρίβεια περίπου ±${Math.round(pos.coords.accuracy || 0)} m`;
    }
  } catch (error) {
...
async function optimizeRealRoute() {
  ...
  const routablePending = pending.filter(
    item => !unresolvedKeys.has(item.customerKey)
  );
    ...
    if (
      ...
      if (
        ...
        const mergedPending = pending.map(item => {
          if (unresolvedKeys.has(item.customerKey)) {
            return item;
          }
...
async function fetchDirectionsForCurrentOrder(
  ...
  if (stillMissing.length) {
    throw new Error(
      "Δεν μπόρεσα να εντοπίσω: " + missingLocationSummary(stillMissing)
    );
  }
...
async function recalculateEtaForCurrentOrder() {
  ...
  try {
    ...
    const unresolvedNote = stillMissing.length
      ? ` • ⚠ ${stillMissing.length} χωρίς ETA (${missingLocationSummary(stillMissing)})`
      : "";
