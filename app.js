...
//
// This does NOT trust VROOM to choose the order.
// ORS gives us the HGV travel-time matrix.
// The app itself builds and improves the sequence.
// ---------------------------------------------------

function routeTravelSeconds(order, matrix) {
  let total = 0;
  let fromIndex = 0; // 0 = current truck position

  for (const customerIndex of order) {
    const seconds = Number(
      matrix[fromIndex]?.[customerIndex]
    );
    ...
    total += seconds;
    fromIndex = customerIndex;
  }

  return total;
}

function nearestNeighbourOrder(matrix, customerCount) {
  const unvisited = [];

  // Matrix index 0 is the truck.
  // Customers are 1..customerCount.
  ...
  while (unvisited.length) {
    ...
    for (let p = 0; p < unvisited.length; p++) {
      const candidate = unvisited[p];
      const seconds = Number(
    ...
    if (bestPos < 0) {
      throw new Error(
        "The truck router cannot connect all customers."
      );
    }

    const next =
      unvisited.splice(bestPos, 1)[0];
...
function twoOptOpenRoute(order, matrix) {
  let best = [...order];
  let bestCost =
    routeTravelSeconds(best, matrix);
...
async function fetchHgvMatrix(position, pending) {
  const key =
    localStorage.getItem(
      API_KEY_STORAGE
    );
  ...
  if (!response.ok) {
    throw new Error(
      ...
      result?.error ||
      result?.message ||
      `Matrix API error ${response.status}`
    );
  }

  if (!result.durations) {
    throw new Error(
      "Truck travel-time matrix was not returned."
    );
  }
...
async function optimizeRealRoute() {
  ...
  el("optimizerStatus")
    .textContent =
      "Calculating truck travel time between every stop…";
  ...
  try {
    ...
    // Current/manual order is matrix indexes 1..N.
    const currentMatrixOrder =
      pending.map(
        (_, index) =>
          index + 1
      );
    ...
    // Build a sensible road-based route and improve it.
    let suggested =
    ...
    if (
      ...
      throw new Error(
        "Could not create a complete truck route."
      );
    }
    ...
    // If the suggested order is identical or not better,
    // do not shuffle the driver's route.
    const sameOrder =
      suggested.every(
        (matrixIndex, i) =>
          matrixIndex ===
          currentMatrixOrder[i]
      );
    ...
    if (
      ...
      el("optimizerStatus")
        .textContent =
          `✓ Your current order is already as good as the matrix suggestion • truck driving ~${formatDurationSeconds(currentSeconds)}`;
    ...
    if (!apply) {
      el("optimizerStatus")
        .textContent =
          "Suggestion rejected. Your current order was kept.";
    ...
    el("optimizerStatus")
      .textContent =
        `✓ Truck-road suggestion applied • estimated driving ~${formatDurationSeconds(suggestedSeconds)}. Check Numbered map, correct with ↑ ↓ if needed, then Recalculate ETA.`;
...
async function recalculateEtaForCurrentOrder() {
  ...
  el("optimizerStatus")
    .textContent =
      "Calculating truck-road ETA for the exact current order…";
  ...
  try {
    ...
    const lastCustomer = last ? customerByKey(last.customerKey) : null;
    const finishEpoch = last?.eta ? Number(last.eta) + Math.max(0, Math.round(Number(lastCustomer?.serviceMin ?? 12) * 60)) : null;
    state.routeStats = {
      distanceMeters,
      fuelLiters: distanceMeters > 0 ? (distanceMeters / 1000) * 0.25 : 0,
      finishEpoch
    };
    saveState();
